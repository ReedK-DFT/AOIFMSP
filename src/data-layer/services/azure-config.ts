...
export type AzureCredentialConfig =
  | AzureDefaultCredentialConfig
  | AzureManagedIdentityConfig
  | AzureClientSecretCredentialConfig;
...
function buildStorageConfig(): AzureStorageServiceConfig {
  const storage: AzureStorageServiceConfig = {};
...
export function loadAzureDataLayerConfigFromEnv(): AzureDataLayerConfig {
  const authMode = (readEnv('AOIFMSP_AZURE_AUTH_MODE') ?? 'default') as AzureDataLayerAuthMode;
  let auth: AzureCredentialConfig;
...
export function resolveStorageEndpoints(storage: AzureStorageServiceConfig): Required<Pick<AzureStorageServiceConfig, 'tableEndpoint' | 'blobEndpoint' | 'queueEndpoint'>> {
